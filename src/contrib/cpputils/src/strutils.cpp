...
std::string str_format(const char* fmt, ...)
{
    ...
    va_list arg;
    va_start(arg, fmt);
    int size = vsnprintf(buf, sizeof(buf), fmt, arg);
    va_end(arg);
    if (size >= (int)sizeof(buf)) {
        large_buf = new char[size + 1];
        va_start(arg, fmt);
        vsnprintf(large_buf, size + 1, fmt, arg);
        va_end(arg);
    }
...
const char* find_first_non(const char* s, const char* removed)
{
    const char* start = s;
    if (removed) {
...
const char* find_last_non(const char* s, const char* removed)
{
    const char* end = s;
    if (removed) {
...
std::string str_replaced_impl(const char *s, const char *from, size_t from_len, const char *to)
{
    std::string ret;
    if (from_len == 0) {
        ret.append(s);
...
void str_replace_fast_impl(std::string& s, const char* from, size_t from_len, const char* to, size_t to_len)
{
    if (from_len == 0)
        return;
...
void str_replace_slow_impl(std::string& s, const char* from, size_t from_len, const char* to, size_t to_len)
{
    if (from_len == 0)
        return;
...
void str_replace_impl(std::string& s, const char* from, size_t from_len, const char* to, size_t to_len)
{
...
void str_split_impl(const char* s, const char* split_pos, std::string* first, std::string* last)
{
    if (split_pos) {
        if (first) {
            first->clear();
