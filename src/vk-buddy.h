...
using BuddyUpdatedCb = std::function<void()>;
...
void update_buddy_list(PurpleConnection* gc, const BuddyUpdatedCb& on_update_cb = nullptr,
                       bool update_presence = false);

// Adds or updates information on buddy with given uid. See update_buddy_list on update_presence.
void update_buddy(PurpleConnection* gc, uint64 uid, const BuddyUpdatedCb& on_update_cb = nullptr,
                  bool update_presence = false);
